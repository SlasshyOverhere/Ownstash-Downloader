...
export function use3DTilt(options: Use3DTiltOptions = {}) {
    ...
    const applyTilt = useCallback((tilt: TiltState) => {
        if (ref.current) {
            ref.current.style.transform = `perspective(${perspective}px) rotateX(${tilt.rotateX}deg) rotateY(${tilt.rotateY}deg) scale(${tilt.scale})`;
        }
    }, [perspective]);
    ...
    const scheduleTiltUpdate = useCallback((nextTilt: TiltState) => {
        ...
        animationFrameRef.current = requestAnimationFrame(() => {
            ...
            applyTilt(latestTiltRef.current);
        });
    }, [applyTilt]);
    ...
    // Omit transform, we do that imperatively to bypass React rendering
    const tiltStyle = {
