...
export function use3DTilt(options: Use3DTiltOptions = {}) {
    ...
    // Removed useState to prevent React re-renders on mouse move
    // Mutating DOM directly for high-frequency updates
    ...
    const scheduleTiltUpdate = useCallback((nextTilt: TiltState) => {
        ...
        animationFrameRef.current = requestAnimationFrame(() => {
            ...
            if (ref.current) {
                const t = latestTiltRef.current;
                ref.current.style.transform = `perspective(${perspective}px) rotateX(${t.rotateX}deg) rotateY(${t.rotateY}deg) scale(${t.scale})`;
            }
        });
    }, [perspective]);
    ...
    // Omit transform to prevent React from overwriting imperative changes
    const tiltStyle = {
