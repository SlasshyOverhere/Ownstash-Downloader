...
export function use3DTilt(options: Use3DTiltOptions = {}) {
    ...
    // Bypass React render cycle by imperatively mutating the DOM element's style.
    // This prevents expensive React re-renders on every high-frequency mouse movement event.
    const applyTransform = useCallback((nextTilt: TiltState) => {
        if (!ref.current) return;
        ref.current.style.transform = `perspective(${perspective}px) rotateX(${nextTilt.rotateX}deg) rotateY(${nextTilt.rotateY}deg) scale(${nextTilt.scale})`;
    }, [perspective]);
