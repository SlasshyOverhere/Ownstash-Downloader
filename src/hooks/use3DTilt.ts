...
export function use3DTilt(options: Use3DTiltOptions = {}) {
    ...
    const applyTilt = useCallback((tiltToApply: TiltState) => {
        if (ref.current) {
            ref.current.style.transform = `perspective(${perspective}px) rotateX(${tiltToApply.rotateX}deg) rotateY(${tiltToApply.rotateY}deg) scale(${tiltToApply.scale})`;
        }
    }, [perspective]);
