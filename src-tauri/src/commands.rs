...
/// If a custom player is configured in settings, use it; otherwise use system default
#[tauri::command]
pub async fn open_with_external_player(state: State<'_, AppState>, file_path: String) -> Result<(), String> {
    ...
    // Securely retrieve the player path from the backend state to avoid arbitrary code execution
