...
/// Fetches the player path securely from the database instead of trusting frontend input
#[tauri::command]
pub async fn open_with_external_player(state: State<'_, AppState>, file_path: String) -> Result<(), String> {
    ...
    let player_path = state.db.lock().map_err(|e| e.to_string())?.get_setting("external_player_path").map_err(|e| e.to_string())?;
