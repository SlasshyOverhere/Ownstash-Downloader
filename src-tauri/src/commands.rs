...
/// Uses custom player from secure backend settings if configured, otherwise uses system default
#[tauri::command]
pub async fn open_with_external_player(
    ...
    match player_path {
        ...
        _ => {
            // Use system default if no player is set or if string is empty
